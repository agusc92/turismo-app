...
class TiposManager extends Component
{
    use WithPagination;
    ...
    {
        return ['tipo' => 'required|string|max:100|unique:tipos,tipo' . ($this->isEditing ? ',' . $this->editingId : '')];
    }
    ...
    public function save(): void
    {
        $data = $this->validate();
        if ($this->isEditing) {
            Tipo::findOrFail($this->editingId)->update($data);
            $this->toast('Tipo actualizado.');
        } else {
            Tipo::create($data);
            $this->toast('Tipo creado.');
        }
    ...
    {
        if ($this->confirmId) Tipo::findOrFail($this->confirmId)->delete();
        $this->toast('Tipo eliminado.');
    ...
    {
        $tipos = Tipo::where('tipo', 'like', '%' . $this->search . '%')
