...
class MenusManager extends Component
{
    use WithPagination;
    ...
    {
        return ['tipo' => 'required|string|max:100|unique:menus,tipo' . ($this->isEditing ? ',' . $this->editingId : '')];
    }
    ...
    public function openEdit(int $id): void
    {
        ...
        $this->editingId = $m->id; $this->tipo = $m->tipo;
    ...
    public function save(): void
    {
        ...
        if ($this->isEditing) {
            ...
            $this->toast('Menú actualizado.');
        } else {
            Menu::create($data);
            $this->toast('Menú creado.');
        }
    ...
    {
        if ($this->confirmId) Menu::findOrFail($this->confirmId)->delete();
        $this->toast('Menú eliminado.');
    ...
    {
        $menus = Menu::where('tipo', 'like', '%' . $this->search . '%')
            ->withCount('gastronomicos')
            ->orderBy('tipo')
            ->paginate(15);

        return view('livewire.admin.menus-manager', compact('menus'));
    }
}
