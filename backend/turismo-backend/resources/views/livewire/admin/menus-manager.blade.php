...
<div class="card">
    <div class="table-wrap">
        <table>
            ...
            <tbody>
                @forelse($menus as $m)
                <tr wire:key="menu-{{ $m->id }}">
                    <td class="muted">{{ $m->id }}</td>
                    <td>
                        <div style="font-weight:600">{{ $m->tipo }}</div>
                    </td>
                    <td>
                        <span class="badge badge-blue">{{ $m->gastronomicos_count }} locales</span>
                    </td>
                    <td>
                        <div style="display:flex;gap:6px">
                            <button class="btn btn-success btn-sm" wire:click="openEdit({{ $m->id }})">✏️ Editar</button>
                            <button class="btn btn-danger btn-sm" wire:click="confirmDelete({{ $m->id }}, '{{ addslashes($m->tipo) }}')">🗑️</button>
                        </div>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="4">
                        <div class="empty-state">
                            ...
                            <div class="empty-text">No hay menús{{ $search ? ' que coincidan con la búsqueda' : '' }}</div>
                        </div>
                    </td>
                </tr>
                @endforelse
...
<div class="modal-overlay" wire:click.self="$set('showModal', false)">
    <div class="modal">
        <div class="modal-header">
            <span class="modal-title">{{ $isEditing ? '✏️ Editar Menú' : '＋ Nuevo Menú' }}</span>
            <button class="modal-close" wire:click="$set('showModal', false)">×</button>
        </div>
        <div class="modal-body">
            <div class="form-grid form-grid-1">
                <div class="form-group">
                    <label>Nombre/Tipo de Menú *</label>
                    <input type="text" wire:model="tipo" placeholder="Ej: Celíacos, Vegano, Infantil…">
        ...
        <div class="modal-footer">
            ...
            <button class="btn btn-primary" wire:click="save" wire:loading.attr="disabled">
                <span wire:loading.remove>{{ $isEditing ? 'Guardar cambios' : 'Crear menú' }}</span>
...
<div class="modal-overlay" wire:click.self="$set('showConfirm', false)">
    <div class="modal confirm-modal">
        <div class="modal-body" style="padding:32px;text-align:center">
            ...
            <div class="modal-title" style="margin-bottom:10px">Eliminar menú</div>
            <div class="confirm-text">
                ¿Seguro que querés eliminar el menú <span class="confirm-name">{{ $confirmName }}</span>?
