...
<div class="card-header" style="background:var(--bg-card);border-radius:var(--radius);border:1px solid var(--border);margin-bottom:20px;">
    <div class="search-bar">
        ...
        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Buscar tipo…">
    </div>
    <button class="btn btn-primary" wire:click="openCreate">
        ＋ Nuevo Tipo Actividad
    </button>
</div>
...
<div class="card">
    <div class="table-wrap">
        <table>
            ...
            <tbody>
                @forelse($tipos as $t)
                <tr wire:key="tipo-{{ $t->id }}">
                    <td class="muted">{{ $t->id }}</td>
                    <td>
                        <div style="font-weight:600">{{ $t->tipo }}</div>
                    </td>
                    <td>
                        <span class="badge badge-blue">{{ $t->actividades_count }} actividades</span>
                    </td>
                    <td>
                ...
                <tr>
                    <td colspan="4">
                        <div class="empty-state">
                            ...
                            <div class="empty-text">No hay tipos de actividad{{ $search ? ' que coincidan con la búsqueda' : '' }}</div>
                        </div>
                    </td>
                </tr>
                @endforelse
...
<div class="modal-overlay" wire:click.self="$set('showModal', false)">
    <div class="modal">
        <div class="modal-header">
            <span class="modal-title">{{ $isEditing ? '✏️ Editar Tipo Actividad' : '＋ Nuevo Tipo Actividad' }}</span>
        ...
        <div class="modal-body">
            <div class="form-grid form-grid-1">
                <div class="form-group">
                    ...
                    <input type="text" wire:model="tipo" placeholder="Ej: Deportes de Aventura">
...
<div class="modal-overlay" wire:click.self="$set('showConfirm', false)">
    <div class="modal confirm-modal">
        <div class="modal-body" style="padding:32px;text-align:center">
            ...
            <div class="modal-title" style="margin-bottom:10px">Eliminar tipo de actividad</div>
